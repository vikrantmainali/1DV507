...
public class FunctionPointers {
	...
	public static void main(String[] args) {
		...
		Predicate<Integer> odd = n -> n % 2 != 0;      //no so that mob 2 of it is equal to 0
		selectAndPrint(list, odd);	

		System.out.print("\nPrint all numbers greater than 10: ");
		Predicate<Integer> aboveTen = n -> n > 10;   // n such that it is greater than 10
	...
	private static List<Double> applyFunction(List<Double> numbers, Function<Double, Double> fx) 
	{
		...
		List<Double> fxNumbers = new ArrayList<Double>();
		Object[] arr = numbers.toArray();
		
		for (double i = 0; i <= numbers.size() - 1; i++)
		{	
			fxNumbers.add((int) i, fx.apply((Double) arr[(int) i]));
		}
