...
public class CountrySort {
	...
	public static void main(String[] args) {
		...
		Comparator<String> longestName = (s1,s2) -> s2.length() - s1.length();       // using subtraction of length to get the longest country first
		...
		Comparator<String> reverseAlphabetic = (s1,s2) -> s2.compareTo(s1);    // using compareTo so that it can bubble sort between the two
		...
		Comparator<String> countAs = (s1,s2) -> (int)
				...
				s1.chars().filter( c-> c == 'A' || c == 'a').count());    //filtering out the and A characters in them 
